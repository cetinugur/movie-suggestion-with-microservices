    ...
    public abstract class Repository<T> : IRepository<T> where T : class, new()
    {
        ...
        public bool SaveChanges()
        {
            try
            {
                ...
                {
                    if (item.Properties.Any(x => x.Metadata.Name == DbDictionary.ConcurrencyStamp))
                        item.Property(DbDictionary.ConcurrencyStamp).CurrentValue = Guid.NewGuid();

                    if (item.Properties.Any(x => x.Metadata.Name == DbDictionary.UpdateDate))
                        item.Property(DbDictionary.UpdateDate).CurrentValue = DateTime.UtcNow;
                }
            ...
            {
                throw new DbUpdateConcurrencyException(MessageDictionary.DbUpdateConcurrency);
            }
        }
